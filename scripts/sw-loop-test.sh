...
if grep -q 'output-format.*json' "$SCRIPT_DIR/sw-loop.sh" || grep 'output-format.*json' "$SCRIPT_DIR/lib/loop-iteration.sh" >/dev/null; then
...
if grep -q '2>"$err_file"' "$SCRIPT_DIR/sw-loop.sh" || grep '2>"$err_file"' "$SCRIPT_DIR/lib/loop-iteration.sh" >/dev/null; then
...
CLAUDE_EOF
    ...
    if echo "$output" | grep -F "LOOP_COMPLETE" >/dev/null; then
        assert_pass "Loop detected completion signal"
    elif echo "$output" | grep -i "complete.*LOOP_COMPLETE\|LOOP_COMPLETE.*accepted" >/dev/null; then
        assert_pass "Loop detected completion signal"
    else
...
CLAUDE_EOF
    ...
    if echo "$output" | grep -E "Iteration [2-9]|iteration [2-9]" >/dev/null; then
        assert_pass "Loop runs multiple iterations when tests fail initially"
    elif echo "$output" | grep "LOOP_COMPLETE" >/dev/null; then
        assert_pass "Loop runs multiple iterations and completes"
    elif echo "$output" | grep -i "circuit breaker\|max iteration" >/dev/null; then
...
CLAUDE_EOF
    ...
    if echo "$output" | grep -iE "max iteration|iteration.*3|Max iterations" >/dev/null; then
...
CLAUDE_EOF
    ...
    if echo "$output" | grep -i "stuckness\|stuck" >/dev/null; then
        assert_pass "Loop detects stuckness"
    elif echo "$output" | grep -i "circuit breaker" >/dev/null; then
        assert_pass "Loop circuit breaker triggered (stuckness-related)"
    elif echo "$output" | grep -i "max iteration" >/dev/null; then
...
exit 0' > "$TEST_TEMP_DIR/bin/claude"
    ...
    if echo "$output" | grep -iE "budget exhausted|Budget exhausted|LOOP BUDGET_EXHAUSTED" >/dev/null; then
...
" 2>/dev/null)
rm -rf "$_valid_tmp"
if echo "$_valid_out" | grep "exit=1" >/dev/null; then
...
CLAUDE_EOF
    ...
    if echo "$output" | grep -iE "Git:|progress|insertion|LOOP_COMPLETE" >/dev/null; then
...
output=$(bash "$SCRIPT_DIR/sw-loop.sh" --help 2>&1 | sed $'s/\033\[[0-9;]*m//g') && rc=0 || rc=$?
if echo "$output" | grep 'additional-test-cmds' >/dev/null; then
