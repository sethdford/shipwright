...
test_doctor_output_counts() {
    ...
    if ! echo "$output" | grep "warnings" >/dev/null; then
        return 1
    fi
    if ! echo "$output" | grep "failed" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_doctor_all_pass() {
    ...
    if ! echo "$output" | grep "0 failed" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_doctor_bad_escape_time() {
    ...
    if ! echo "$output" | grep "escape-time.*500" >/dev/null; then
        return 1
    fi
...
test_doctor_bad_history() {
    ...
    if ! echo "$output" | grep "history-limit.*2000" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_doctor_passthrough_off() {
    ...
    if ! echo "$output" | grep "allow-passthrough.*off" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_doctor_version_check() {
    ...
    if ! echo "$output" | grep "3.4" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_doctor_terminal_detect() {
    ...
    if ! echo "$output" | grep "Ghostty" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_install_tpm_already_exists() {
    ...
    if ! echo "$output" | grep "already installed" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_fix_applies_fixes() {
    ...
    if ! echo "$output" | grep "Fixed" >/dev/null; then
        return 1
    fi
...
test_fix_no_fixes_needed() {
    ...
    if ! echo "$output" | grep "No fixes needed\|already optimized" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_reload_no_conf() {
    ...
    if ! echo "$output" | grep "No.*tmux.conf" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_cli_routes_doctor() {
    ...
    if ! echo "$output" | grep "tmux Doctor" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_cli_aliases() {
    ...
    if ! echo "$output" | grep "Plugin Installer" >/dev/null; then
        return 1
    fi
...
test_doctor_outside_tmux() {
    ...
    if ! echo "$output" | grep "config file\|\.tmux\.conf" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_doctor_missing_tpm() {
    ...
    if ! echo "$output" | grep "TPM not installed" >/dev/null; then
        return 1
    fi
    return 0
}
...
test_doctor_old_tmux_version() {
    ...
    if ! echo "$output" | grep "3.3.*recommended\|3.2" >/dev/null; then
        return 1
    fi
    return 0
}
