...
| Key                                | Default | Purpose                                                                                        |
| ---------------------------------- | ------- | ---------------------------------------------------------------------------------------------- |
| `circuit_breaker_threshold`        | `4`     | Base threshold: consecutive failures before circuit breaker trips                              |
| `adaptive_circuit_breaker_enabled` | `0`     | Enable adaptive thresholding based on failure signature similarity (0 = disabled, 1 = enabled) |
...
When `loop.adaptive_circuit_breaker_enabled=1`, the circuit breaker threshold adapts dynamically based on failure signature similarity:

- **Similar failures** (same error type, message, stage): threshold increases by +2
  - Example: "TypeError in test stage" recurring → allow 6 attempts instead of 4 to fix the root cause
- **Diverse failures** (different error types): threshold decreases by -1
  - Example: "TypeError", then "ENOENT", then "timeout" → trip faster at 3 instead of 4

The algorithm extracts error signatures from `.claude/pipeline-artifacts/error-log.jsonl` and compares the last two failures:

- Type match (same error class): +30%
- Error message match (first 50 chars): +40%
- Stage match (same pipeline phase): +10%
- Timestamp proximity (< 2 min): +20%
